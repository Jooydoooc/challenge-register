...
const FIELDS = [
  ...
  { key: 'group', label: 'Group', required: true, max: 80 },
  { key: 'level', label: 'English level', required: true, max: 40 },
  { key: 'currentBand', label: 'Current band', required: true, max: 20 },
  { key: 'targetBand', label: 'Target band', required: true, max: 20 },
  { key: 'source', label: 'Heard via', required: false, max: 40 },
  { key: 'notes', label: 'Notes', required: false, max: 500 },
];

/**
 * Closed sets for the fields the page renders as <select>. The browser can send
 * anything, so these are checked here too. Keep in step with index.html.
 */
const LEVELS = [
  'Beginner', 'Elementary', 'Pre-Intermediate',
  'Intermediate', 'Upper-Intermediate', 'Advanced', 'Not sure',
];
const SOURCES = [
  'Instagram', 'Telegram channel', 'A friend or former student',
  'Google search', 'YouTube', 'Other',
];
const BANDS = ['Never taken the test'];
for (let b = 4.0; b <= 9.0 + 1e-9; b += 0.5) BANDS.push(b.toFixed(1));
...
function validate(payload) {
  ...
  // Unicode letters are required: group names like "O‘zbek guruhi" or
  // "Teacher's Group" are ordinary here. Length is already capped by FIELDS.
  if (data.group && !/^[\p{L}\p{N} '’‘"–—().,:/&+-]+$/u.test(data.group)) {
    errors.push('Group is not valid');
  }
  if (data.level && !LEVELS.includes(data.level)) {
    errors.push('English level is not valid');
  }
  if (data.currentBand && !BANDS.includes(data.currentBand)) {
    errors.push('Current band is not valid');
  }
  if (data.targetBand && !BANDS.includes(data.targetBand)) {
    errors.push('Target band is not valid');
  }
...
function buildMessage(data, request) {
  ...
  lines.push(
    '',
    `<b>Group:</b> ${esc(data.group)}`,
    `<b>Level:</b> ${esc(data.level)}`,
    `<b>Band:</b> ${esc(data.currentBand)} → ${esc(data.targetBand)}`
  );
